...
TEST_F(DoublePendulum, IrisZoDoublePendulum) {
  IrisZoOptions options;
  options.verbose = true;
  options.meshcat = meshcat;
  HPolyhedron region = IrisZoFromUrdf(urdf, starting_ellipsoid, options);
  run_checks_on_region(region);
  plot_env_and_region(region);
  ...
  region = IrisZoFromUrdf(urdf, starting_ellipsoid, options, &domain);
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      IrisZoFromUrdf(urdf, starting_ellipsoid, options, &domain),
      ".*wrong dimension.*");
}
